...
export const Dashboard = () => {
  ...
  const updateTicketStatus = async (ticketId: string, status: string, notes: string) => {
    try {
      await ticketAPI.update(ticketId, { status: status as 'open' | 'in-progress' | 'resolved', notes });
      await refetch();
    } catch (error) {
    ...
    <>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div>
                ...
                <p className="text-gray-600 mt-1">Support Ticket Management System</p>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-10">
            {[
              ...
              <div key={index} className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-white/50 group">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600 uppercase tracking-wide">{stat.label}</p>
                    <p className="text-3xl font-bold text-gray-900 mt-1">{stat.value}</p>
                  </div>
                  <div className={`text-3xl p-3 rounded-2xl bg-gradient-to-br ${stat.color} shadow-lg group-hover:scale-110 transition-transform`}>{stat.icon}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 mb-8 border border-white/50 shadow-lg">
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => { setFilter('all' as const); setSearchTerm(''); }}
      ...
      {selectedTicket && (
        <StatusModal
          ...
          onUpdate={(status, notes) => updateTicketStatus(selectedTicket.id, status, notes)}
        />
      )}
    </>
  );
